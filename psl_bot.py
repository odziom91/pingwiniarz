...
rss_channel = int(config.get("config","rss_id"))
...
async def on_command_error(ctx, error):
    ...
    try:
        ...
        await psl_Exception(client, err_channel, str(e))
...
async def chg_presence():
    komunikaty = open(f'psl_presence.txt', 'r').read().splitlines()
    await client.change_presence(activity=discord.Game(name=komunikaty[randint(0, len(komunikaty)-1)]))
...
if __name__ == '__main__':
    ...
    client.add_cog(psl_RssParser(client, rss_channel, embed_color))
    client.run(token)
